...
(deftest kill-future-test ()
  (with-fixture future-default-thread-pool
    ...
    (let* ((f (future (sleep 1) (+ 1 1))))
      ...
      (is (future::queue-empty-p future::*finished-futures*)))
    (let* ((f1 (future (sleep 0.2) (+ 1 1)))
      ...
      (sleep 0.2)
      (is (= 1 (running-futures-count)))
      (is (= 1 (future::queue-length future::*finished-futures*)))
      (kill-future f1)
      (is (= 1 (running-futures-count)))
      (is (= 0 (future::queue-length future::*finished-futures*)))
      (kill-future f2)
      (assert-no-futures)
      (is (future::future-finished-p f1))
...
(deftest kill-all-futures-test ()
  (with-fixture future-default-thread-pool
    ...
    (let* ((f1 (future (sleep 0.1) (+ 1 1)))
      ...
      (is (= 0 (future::queue-length future::*finished-futures*)))
      (sleep 0.3)
      (is (= 2 (future::queue-length future::*finished-futures*)))
      (kill-all-futures)
      (assert-no-futures)
      (is (= 0 (future::queue-length future::*finished-futures*))))
    (let* ((f1 (future (sleep 0.2) (+ 1 1)))
